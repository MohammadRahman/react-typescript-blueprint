...
const EmailAccountTable = ({ onEdit, status }: EmailAccountTableProps) => {
  ...
  function handleTypeSortsAndFilter() {
    const filterKey = filterValue.type
      ? "type"
      : filterValue.email
        ? "email"
    ...
    const filterValueToUse =
      filterValue.type || filterValue.email || filterValue.displayName || filterValue.imapEmail;
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      header: "Protocl",
      accessorFn: row => (row.securityProtocol === 1 ? "TLS" : "SSL"), // Map `1` to `TLS`, other values to `Unknown`
      size: 150,
    },
    {

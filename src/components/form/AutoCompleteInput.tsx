...
const InputWithAutocomplete: React.FC<{
  ...
  rules?: any;
  fieldValues: string[];
}> = ({ control, name, placeholder, bgc, rules, fieldValues }) => {
  ...
  const nestedOptions = useMemo(() => ({ Model: fieldValues }), [fieldValues]);
      ...
      render={({ field: { onChange, value } }) => {
        ...
        const handleOptionSelect = (option: string) => {
          ...
          if (textBeforeCursor.includes("@Model.")) {
            newValue = value.replace(/@Model\.[^.]*$/, `@Model.${option}`);
          } else if (textBeforeCursor.includes("@")) {
            newValue = value.replace(/@[^@]*$/, `@${option}`);
          }
        ...
        return (
          <div style={{ position: "relative" }}>
            ...
            />
            {showDropdown && (
              <Dropdown style={{ top: dropdownPosition.top, left: dropdownPosition.left }}>
...
//   fieldValues: string[];
//   error?: string;
// }> = ({ value, onChange, placeholder, bgc, fieldValues, error }) => {
//   const [showDropdown, setShowDropdown] = useState(false);
//   const [_, setDropdownOptions] = useState<string[]>([]);
//   const [dropdownPosition, setDropdownPosition] = useState({ top: 0, left: 0 });
//   const inputRef = useRef<HTMLInputElement>(null);
